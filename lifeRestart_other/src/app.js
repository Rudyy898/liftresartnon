...
class App{
    ...
    initPages() {
        ...
        <div id="main">
            ...
            <div id="title">
            人生重开模拟器超级修改死不了版<br>
                <div style="font-size:1.5rem; font-weight:normal;">（玩之前大喊bug是神，媚媚是bug最好最美的老婆）</div>
            </div>
        ...
        <div id="main">
            ...
            <button id="random" class="mainbtn" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);"">随便抽！</button>
            <ul id="talents" class="selectlist"></ul>
            <button id="next" class="mainbtn">想选几个就几个（选多了会挡住属性）</button>
        </div>
        `);
        ...
        groups.CHR = getBtnGroups("颜值", 0, Infinity); // 颜值 charm CHR
        groups.INT = getBtnGroups("智力", 0, Infinity); // 智力 intelligence INT
        groups.STR = getBtnGroups("体质", 0, Infinity); // 体质 strength STR
        groups.MNY = getBtnGroups("家境", 0, Infinity); // 家境 money MNY
        ...
        propertyPage
            ...
            .click(()=>{
                let t = this.#totalMax;
                const arr = [this.#totalMax, this.#totalMax, this.#totalMax, this.#totalMax];
                while(t>0) {
                    const sub = Math.round(Math.random() * (Math.min(t, this.#totalMax) - 1)) + 1;
                    while(true) {
                ...
                groups.CHR.set(this.#totalMax - arr[0]);
                groups.INT.set(this.#totalMax - arr[1]);
                groups.STR.set(this.#totalMax - arr[2]);
                groups.MNY.set(this.#totalMax - arr[3]);
            });
